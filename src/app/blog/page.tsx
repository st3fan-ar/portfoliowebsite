  ...
  return (
    <main className="min-h-screen bg-[var(--bg-primary)] py-24 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-xl text-[var(--text-secondary)] mb-16">Insights on Power Electronics, Engineering Leadership, and Green Hydrogen</p>
        {posts.length === 0 ? (
            ...
            {posts.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`}>
                <article className="bg-[var(--bg-secondary)]/50 border border-[#e5e4dc] rounded-2xl p-8 hover:border-[var(--accent)] transition-all group">
                  ...
                  <h2 className="font-heading text-2xl md:text-3xl font-bold mb-4 group-hover:text-[var(--accent)] transition-colors">{post.title}</h2>
